...
# функция объединения дублирующихся записей контактов

def dublicate_filter(contacts: list):
...
# функция приведения номеров телефонов к единому формату
def phone_format(contact_list: list):
    ...
    return dublicate_filter(new_list)


pprint(phone_format(contacts_list))
format_list = phone_format(contacts_list)
